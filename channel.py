...
def channel_forward2(x,ratio):
    temp = x.reshape(x.shape[0],(int)(x.shape[1]*x.shape[2]*x.shape[2]/2),2,1)
    #print(temp.shape)
    h = torch.randn(x.shape[0],1,2,2)
    h[:,:,1,0] = -h[:,:,0,1]
    h[:,:,1,1] = h[:,:,0,0]
    #print(h.shape)
    temp = torch.matmul(h,temp) * np.sqrt(1/2) #记得乘上归一化因子
    #print(temp.shape)
    x = temp.reshape(x.shape)
    #print(x.shape)
    return (x + ratio * torch.randn_like(x)) #信噪比为SNR，可指